...
async def on_ready():
    ...
    if sys.maxsize > 2**32:
        opus.load_opus('/usr/lib/x86_64-linux-gnu/libopus.so')
    else:
...
async def on_message(msg):
    ...
    if msg.author.id != client.user.id:  # ignore our own commands
        if msg.content.startswith(conf['invoker']):
            g_1 = msg.content.lower()[len(conf['invoker']):]
            if g_1 == 'help':
                g_4 = '\n' + conf['invoker']
                g_3 = '{0}git\n{0}{1}'.format(
                    conf['invoker'], g_4.join(os.listdir('sound/')))
                g_3 = g_3.lower().replace(conf['fileformat'], '')
                g_2 = 'Commands for ' + conf['bot'] + ':\n' + \
